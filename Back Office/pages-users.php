...
if (isset($_GET['idd'])) {
  ...
  $utilisateur->supprimeruser($_GET["idd"]);
}
if (isset($_GET['idb'])) {

  $utilisateur = new user();
  $donnee = $utilisateur->getUserById($_GET['idb']);
  $utilisateur->updateusera($_GET["idb"],$donnee['prenom'],$donnee['nom'],$donnee['age'],$donnee['email'],$donnee['mdp'],$donnee['fonction'],"ban");
  
}
if (isset($_GET['idub'])) {

  $utilisateur = new user();
  $donnee = $utilisateur->getUserById($_GET['idub']);
  $utilisateur->updateusera($_GET["idub"],$donnee['prenom'],$donnee['nom'],$donnee['age'],$donnee['email'],$donnee['mdp'],$donnee['fonction'],"actif");
...
<!DOCTYPE html>
<html lang="en">
  <!-- Mirrored from coderthemes.com/konrix/layouts/pages-starter.php by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 29 Mar 2024 19:44:30 GMT -->
  <head>
    <meta charset="utf-8" />
  ...
  <body>
          ...
          <ul class="menu" data-fc-type="accordion">
            ...
            <li class="menu-item">
              ...
              >
                ...
                <span class="menu-text">Gestion users</span>
                <span class="menu-arrow"></span>
              </a>
    ...
    <button id="sortAgeBtn" class="bg-red-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-sm">
    Trier par âge
</button>
    <table class="table w-full bg-white rounded shadow-lg">
        ...
        <tbody>
            ...
            $listeUtilisateurs = $userInstance->listeruser();

            // Vérifier s'il y a des utilisateurs dans la liste
            if ($listeUtilisateurs) {
                $count = 1; // Pour numéroter les utilisateurs
                foreach ($listeUtilisateurs as $utilisateur) {
                    echo "<tr>";
                    echo "<td class='px-4 py-2'>$count</td>";
                    echo "<td class='px-4 py-2'>" . $utilisateur['prenom'] . "</td>";
                    echo "<td class='px-4 py-2'>" . $utilisateur['nom'] . "</td>";
                    echo "<td class='px-4 py-2'>" . $utilisateur['age'] . "</td>";
                    echo "<td class='px-4 py-2'>" . $utilisateur['email'] . "</td>";
                    echo "<td class='px-4 py-2'>" . $utilisateur['mdp'] . "</td>";
                    echo "<td class='px-4 py-2'>" . $utilisateur['fonction'] . "</td>";
                    echo "<td class='px-4 py-2'>" . $utilisateur['etat'] . "</td>";
                    echo "<td class='px-4 py-2'>";
                    // Bouton Update User
                    echo "<a href='pages-updateusers.php?id=" . $utilisateur['id'] . "' class='bg-green-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded ml-2'>Update User</a>";
                    // Bouton Delete User
                    ...
                    echo "<a href='pages-users.php?idd=" . $utilisateur['id'] . "' class='bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded ml-2'>Delete User</a>";
                    // Bouton Ban User
                    echo "<a href='pages-users.php?idb=" . $utilisateur['id'] . "' class='bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded ml-2'>Ban User</a>";
                    echo "<a href='pages-users.php?idub=" . $utilisateur['id'] . "' class='bg-green-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded ml-2'>Unban User</a>";
                    echo "</td>";
                    echo "</tr>";
                    $count++;
                }
...
<div class="flex justify-between items-center mb-4">
    <div>
        <select id="perPageSelect" class="bg-white border border-gray-300 rounded px-3 py-1">
    ...
    <div>
        <button id="prevPageBtn" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l">Précédent</button>
        <button id="nextPageBtn" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-r">Suivant</button>
    </div>
</div>
<script>
    ...
    let sortAscending = true; // Variable pour suivre l'état du tri

    // Fonction de tri par âge
    document.getElementById("sortAgeBtn").addEventListener("click", function() {
        let rows = Array.from(document.querySelectorAll("tbody tr"));

        rows.sort((a, b) => {
            let ageA = parseInt(a.querySelector("td:nth-child(4)").textContent.trim());
            let ageB = parseInt(b.querySelector("td:nth-child(4)").textContent.trim());
            if (sortAscending) {
                return ageA - ageB; // Tri croissant
            } else {
                return ageB - ageA; // Tri décroissant
            }
        });

        // Rafraîchissement du tableau avec les lignes triées
        ...
        // Inversion de l'état du tri pour le prochain clic
    ...
    // Fonction pour afficher les éléments appropriés selon la pagination
    ...
    // Événement lorsque l'utilisateur change le nombre d'éléments par page
    document.getElementById("perPageSelect").addEventListener("change", function() {
        perPage = parseInt(this.value);
        currentPage = 1; // Réinitialiser la page actuelle à 1 lors du changement de nombre d'éléments par page
        displayItems();
    });

    // Événement lorsque l'utilisateur clique sur le bouton précédent
    ...
    // Événement lorsque l'utilisateur clique sur le bouton suivant
    document.getElementById("nextPageBtn").addEventListener("click", function() {
        let rows = document.querySelectorAll("tbody tr");
        let maxPage = Math.ceil(rows.length / perPage);
